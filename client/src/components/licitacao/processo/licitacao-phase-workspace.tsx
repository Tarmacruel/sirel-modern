...
export interface PhaseWorkspaceItem {
  category: string;
  label: string;
  description?: string;
  concluido: boolean;
...
export interface LicitacaoPhaseWorkspaceProps {
  items: PhaseWorkspaceItem[];
...
const publicationTabs = [
  { value: "documents", label: "Documentos", icon: FileText },
  { value: "schedule", label: "Cronograma", icon: CalendarClock },
  { value: "channels", label: "Canais e dados", icon: Link2 },
] as const;
...
function preferredItem(items: PhaseWorkspaceItem[]) {
  return items.find((item) => !item.concluido) ?? items[0];
}

export function LicitacaoPhaseWorkspace({
  items,
  activeCategory,
  onSelectCategory,
  editor,
  ...
  return (
    ...
    >
      ...
      <div
        ...
        role="tabpanel"
        aria-labelledby={`${id}-tab-schedule`}
      ...
      <div
        id={`${id}-channels-panel`}
        role="tabpanel"
        aria-labelledby={`${id}-tab-channels`}

      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <p className="text-sm font-semibold text-[var(--text-primary)]">
            {singleAgent ? "Responsável" : "Composicao"}
          </p>
          <p className="text-xs text-[var(--text-secondary)]">
            {singleAgent
              ? "Selecione a pessoa designada no ato."
        ...
        <div className="space-y-2">
          {value.map((member, index) => (
            <div
              ...
              className={`grid gap-2 rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-soft)] p-3 ${singleAgent ? "grid-cols-1" : "lg:grid-cols-[minmax(220px,1fr)_190px_90px_120px_40px]"}`}
            >
              <AsyncCombobox<PessoaOption>
                ...
                allowClear
                ariaLabel={
                  singleAgent ? "Agente de contratação" : "Pessoa do membro"
                }
              />
              {!singleAgent && (
                <>
                  <Select
                    ...
                    onChange={(event) =>
                      updateMember(index, {
                        funcao: event.target
                          .value as GrupoInstitucionalMembroFuncao,
                      })
                    }

...
function formatReportValue(key: string, value: unknown) {
  ...
  if (typeof value === "number") {
    return key.toLowerCase().includes("valor")
  ...
  if (typeof value === "string") {
    ...
    if (!Number.isNaN(asDate.getTime()) && value.includes("T"))
      return formatShortDateTimeBR(asDate);
    return value;
  }
...
export function RelatoriosPage() {
  ...
  const [exporting, setExporting] = useState<null | "xlsx" | "pdf" | "print">(
    null,
  );
  ...
  const queryInput = useMemo(
    () => ({
      ...
      secretariaId: filters.secretariaId
        ? Number(filters.secretariaId)
        : undefined,
      modalidadeId: filters.modalidadeId
        ? Number(filters.modalidadeId)
        : undefined,
  ...
  const selectedSecretariaLine = useMemo(() => {
    if (!filters.secretariaId) return undefined;
    return (
      catalogQuery.data?.secretarias.find(
        (item) => String(item.id) === filters.secretariaId,
      )?.nome ?? undefined
    );
  }, [catalogQuery.data?.secretarias, filters.secretariaId]);

  function handleExportCsv() {
    if (!report) return;
    exportReportToCsv(
      `sirel-${filters.tipo.toLowerCase()}.csv`,
      report.columns,
  ...
  function handlePrint() {
    ...
    try {
      openPrintableReport(
        ...
        report.rows as Record<string, unknown>[],
        report.summary,
        { secondaryLine: selectedSecretariaLine },
      );
    } finally {
      setExporting(null);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      >
            ...
            <Select
              ...
              onChange={(event) =>
                setFilters((current) => ({
                  ...current,
                  tipo: event.target
                    .value as (typeof relatorioTipoOptions)[number],
                }))
              }
            >
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          ...
          <Button
            ...
            disabled={!report?.rows.length || exporting !== null}
          >
      ...
      {reportQuery.error ? (
        <Alert variant="error">Falha ao gerar o relatório solicitado.</Alert>
      ) : null}
      ...
      >
        {reportQuery.isLoading ? (
          ...
          <div className="space-y-5">
              ...
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                {report.summary.map((item) => (
                  ...
                  >
                    ...
                    <p className="mt-2 text-xl font-black text-[var(--color-primary-900)]">
                      {formatReportValue(item.label, item.value)}
                    </p>
                  </article>
                ))}
              </div>
            ...
            <div className="rounded-[28px] border border-[rgba(204,225,255,0.92)] bg-[linear-gradient(180deg,rgba(230,240,255,0.52),rgba(255,255,255,0.96))] px-4 py-3 text-sm text-[var(--color-neutral-600)]">
              ...
              <span className="font-semibold text-[var(--color-primary-900)]">
                {formatShortDateTimeBR(report.generatedAt)}
              </span>
            </div>
              ...
              <Table>
                ...
                <TableBody>
                  {report.rows.map((row, index) => (
                    <TableRow
                      key={`${index}-${String((row as Record<string, unknown>)[report.columns[0]?.key] ?? index)}`}
                    >
                      {report.columns.map((column) => (
                  ...
                  {!report.rows.length ? (
                    <TableRow>
                      <TableCell
                        colSpan={Math.max(1, report.columns.length)}
              ...
              <div className="rounded-[28px] border border-dashed border-[rgba(47,84,196,0.28)] bg-white px-5 py-4">
                ...
                <div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-3">
                  {report.summary.map((item) => (
                    <div
                      key={item.label}
                      className="flex items-center justify-between gap-3 rounded-2xl bg-[var(--color-primary-50)] px-4 py-3 text-sm"
                    >
                      <span className="font-semibold text-[var(--color-neutral-700)]">
                        {item.label}
                      </span>
                      <span className="font-black text-[var(--color-primary-900)]">
                        {formatReportValue(item.label, item.value)}
                      </span>
                    </div>
                  ))}
          ...
          <Alert variant="info">
            Selecione os filtros e gere o relatório desejado.
          </Alert>
        )}
      </SectionCard>
    </div>

...
export function ProcessosPage({ processoId }: ProcessosPageProps = {}) {
  ...
  const summaryQuery = trpc.processos.summary.useQuery(undefined, {
    retry: false,
  });
  const listQuery = trpc.processos.list.useQuery(filters, {
    retry: false,
    placeholderData: (previous) => previous,
  });
  const rows = listQuery.data?.items ?? [];
  const displayedRows = selectedProcessId
    ? rows.filter((row) => row.id === selectedProcessId)
    : rows;
  ...
  useEffect(() => {
    ...
    if (
      selectedProcessId &&
      !rows.some((row) => row.id === selectedProcessId)
    ) {
      setSelectedProcessId(null);
    }
  ...
  const setAtivoMutation = trpc.processos.setAtivo.useMutation({
    ...
    onError: (error) =>
      setPageFeedback({ variant: "error", message: error.message }),
  });
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
          {[
            ...
            { label: "Publicados", value: summaryQuery.data?.publicados },
            {
              ...
              value: summaryQuery.data?.paradosHaMaisDeSeteDias,
            },
            {
              label: "Média de paralisação",
              value: summaryQuery.data
                ? `${summaryQuery.data.mediaDiasParado} dias`
                : undefined,
            },
          ].map((card) => (
            <article
              key={card.label}
              className="rounded-3xl border border-[var(--border-subtle)] bg-[var(--surface-soft)] px-4 py-4 shadow-[var(--shadow-card)]"
            >
        ...
        {pageFeedback ? (
          <Alert className="mt-4" variant={pageFeedback.variant}>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="relative min-w-[220px]">
              ...
              <Input
                ...
                placeholder="Buscar por SIREL, protocolo, administrativo, edital ou objeto"
                className="pl-9"
              />
            </div>
            <Select
            ...
            <Select
              value={origemFluxo}
              onChange={(event) =>
                setOrigemFluxo(event.target.value as typeof origemFluxo)
              }
              className="max-w-[180px]"
            >
            ...
            <Select
              value={ativoFilter}
              onChange={(event) =>
                setAtivoFilter(event.target.value as typeof ativoFilter)
              }
            ...
            <label className="flex items-center gap-2 rounded-[18px] border border-[var(--border-subtle)] bg-[var(--surface-card)] px-3 py-2 text-sm text-[var(--text-secondary)]">
              <Checkbox
                checked={somenteParados}
                onChange={(event) => setSomenteParados(event.target.checked)}
              />
                    ...
                    <TableRow
                      ...
                      className={
                        row.id === selectedProcessId
                          ? "cursor-pointer bg-[var(--surface-highlight)]"
                    ...
                    >
                      <TableCell>
                        ...
                        <div className="text-xs text-[var(--text-muted)]">
                          {[
                            ...
                            .filter(Boolean)
                            .join(" • ") ||
                            "Sem protocolo, administrativo ou edital"}
                        </div>
                        <div className="text-xs text-[var(--text-muted)]">
                          Entrada na licitação:{" "}
                          {formatShortDateBR(row.dataEntradaLicitacao)}
                        </div>
                        ...
                        {row.foraDoFluxo ? (
                          <span className="mt-2 inline-flex rounded-full bg-amber-100 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.16em] text-amber-800">
                            Fora do fluxo
                          </span>
                        ) : null}
                      </TableCell>
                      ...
                      <TableCell>
                        <div className="font-semibold text-[var(--text-primary)]">
                          {cleanDisplayText(row.moduloAtual ?? "Sem workflow")}
                        </div>
                        <div className="text-xs text-[var(--text-muted)]">
                          {cleanDisplayText(row.situacao ?? "Sem situação")}
                        </div>
                      </TableCell>
                      <TableCell>
                        {cleanDisplayText(row.etapaAtual ?? "Cadastro inicial")}
                      </TableCell>
                      <TableCell>
                        <span
                          className={[
                      ...
                      <TableCell>
                        {row.contratosAtivos}/{row.contratos}
                      </TableCell>
                    </TableRow>
                  ))}
              {!listQuery.isLoading && !displayedRows.length ? (
                <TableRow>
                  <TableCell
                    colSpan={8}
      ...
      {selectedProcessId ? (
          ...
          action={
            overviewQuery.data?.processo ? (
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Link href={`/dossie/${overviewQuery.data.processo.id}`}>
                  <Button
                    type="button"
                    variant="outline"
                    size="sm"
                ...
                <Button
                  variant={
                    overviewQuery.data.processo.ativo
                      ? "destructive"
                      : "outline"
                  }
                  onClick={() =>
        ...
        >
          {overviewQuery.isLoading ? (
            ...
            <Alert variant="error">
              Falha ao carregar o resumo do processo selecionado.
            </Alert>
          ) : !overviewQuery.data ? (
            ...
            <div className="space-y-6">
              <article className="rounded-[28px] border border-[rgba(204,225,255,0.92)] bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(230,240,255,0.78))] px-5 py-5 shadow-[0_12px_24px_-24px_rgba(15,26,109,0.2)]">
                <div className="flex flex-wrap items-center gap-2">
                  <h4 className="text-xl font-black text-[var(--color-primary-900)]">
                    {overviewQuery.data.processo.numeroSirel}
                  </h4>
                  {overviewQuery.data.processo.foraDoFluxo ? (
                ...
                <p className="mt-2 text-xs font-semibold uppercase tracking-[0.14em] text-[var(--color-primary-700)]">
                  {[
                    overviewQuery.data.processo.protocolo
                      ? `Protocolo ${overviewQuery.data.processo.protocolo}`
                      : null,
                    overviewQuery.data.processo.numeroAdministrativo
                      ? `Adm ${overviewQuery.data.processo.numeroAdministrativo}`
                      : null,
                    overviewQuery.data.processo.numeroEdital
                      ? `Edital ${overviewQuery.data.processo.numeroEdital}`
                      : null,
                  ]
                    .filter(Boolean)
                    .join(" • ") ||
                    "Identificadores complementares ainda não informados"}
                </p>
                <p className="mt-2 text-sm leading-6 text-[var(--color-neutral-700)]">
                  {cleanDisplayText(overviewQuery.data.processo.objeto)}
                </p>
              </article>

              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
                <article className="rounded-[28px] border border-[rgba(204,225,255,0.92)] bg-white px-4 py-4 shadow-[0_12px_24px_-26px_rgba(15,26,109,0.22)] xl:col-span-2">
                  ...
                  <p className="mt-2 text-lg font-black text-[var(--color-primary-900)]">
                    {cleanDisplayText(
                      overviewQuery.data.workflow?.moduloAtual ??
                        "Sem workflow",
                    )}
                  </p>
                  <p className="mt-1 text-sm text-[var(--color-neutral-600)]">
                    {cleanDisplayText(
                      overviewQuery.data.workflow?.etapaAtual ??
                        "Cadastro inicial",
                    )}
                  </p>
                </article>
                <article className="rounded-[28px] border border-[rgba(204,225,255,0.92)] bg-white px-4 py-4 shadow-[0_12px_24px_-26px_rgba(15,26,109,0.22)] xl:col-span-2">
                  ...
                  <p className="mt-2 text-lg font-black text-[var(--color-primary-900)]">
                    {overviewQuery.data.gerencial.diasParado} dia(s)
                  </p>
                  <p className="mt-1 text-sm text-[var(--color-neutral-600)]">
                    Atualizado por último em{" "}
                    {formatShortDateTimeBR(
                      overviewQuery.data.workflow?.atualizadoEm ??
                        overviewQuery.data.processo.atualizadoEm,
                    )}
                  </p>
                </article>
                <article className="rounded-[28px] border border-[rgba(204,225,255,0.92)] bg-white px-4 py-4 shadow-[0_12px_24px_-26px_rgba(15,26,109,0.22)]">
                  ...
                  <p className="mt-2 text-2xl font-black text-[var(--color-primary-900)]">
                    {overviewQuery.data.gerencial.itens}
                  </p>
                </article>
              </div>
              ...
              <div className="grid gap-3 md:grid-cols-3">
                <article className="rounded-[28px] border border-[rgba(204,225,255,0.92)] bg-white px-4 py-4 shadow-[0_12px_24px_-26px_rgba(15,26,109,0.22)]">
                  ...
                  <p className="mt-2 text-2xl font-black text-[var(--color-primary-900)]">
                    {overviewQuery.data.gerencial.documentos}
                  </p>
                </article>
                <article className="rounded-[28px] border border-[rgba(204,225,255,0.92)] bg-white px-4 py-4 shadow-[0_12px_24px_-26px_rgba(15,26,109,0.22)]">
                  <p className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--color-primary-600)]">
                  ...
                  <p className="mt-2 text-2xl font-black text-[var(--color-primary-900)]">
                    {overviewQuery.data.gerencial.contratosAtivos}/
                    {overviewQuery.data.gerencial.contratos}
                  </p>
                </article>
                <article className="rounded-[28px] border border-[rgba(204,225,255,0.92)] bg-white px-4 py-4 shadow-[0_12px_24px_-26px_rgba(15,26,109,0.22)]">
                  <p className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--color-primary-600)]">
                  ...
                  <p className="mt-2 text-2xl font-black text-[var(--color-primary-900)]">
                    {formatShortDateBR(
                      overviewQuery.data.processo.dataEntradaLicitacao,
                    )}
                  </p>
                </article>
                <article className="rounded-[28px] border border-[rgba(204,225,255,0.92)] bg-white px-4 py-4 shadow-[0_12px_24px_-26px_rgba(15,26,109,0.22)]">
                  ...
                  <p className="mt-2 text-2xl font-black text-[var(--color-primary-900)]">
                    {formatShortDateBR(
                      overviewQuery.data.processo.dataAbertura,
                    )}
                  </p>
                </article>
              </div>
              ...
              <article className="rounded-[28px] border border-[rgba(204,225,255,0.92)] bg-white px-4 py-4 shadow-[0_12px_24px_-26px_rgba(15,26,109,0.22)]">
                ...
                <dl className="mt-3 grid gap-3 text-sm text-[var(--color-neutral-700)] md:grid-cols-2">
                  <div className="flex items-center justify-between gap-4 border-b border-[var(--color-neutral-100)] pb-2">
                  ...
                  <div className="flex items-center justify-between gap-4 border-b border-[var(--color-neutral-100)] pb-2">
                    ...
                    <dd className="font-semibold text-[var(--color-primary-900)]">
                      {cleanDisplayText(
                        overviewQuery.data.processo.secretaria.nome,
                      )}
                    </dd>
                  </div>
                  <div className="flex items-center justify-between gap-4 border-b border-[var(--color-neutral-100)] pb-2">
                    ...
                    <dd className="font-semibold text-[var(--color-primary-900)]">
                      {cleanDisplayText(
                        overviewQuery.data.processo.modalidade?.nome ??
                          "Não informada",
                      )}
                    </dd>
                  </div>
                  <div className="flex items-center justify-between gap-4 border-b border-[var(--color-neutral-100)] pb-2">
                    ...
                    <dd className="font-semibold text-[var(--color-primary-900)]">
                      {cleanDisplayText(
                        overviewQuery.data.processo.statusAtual?.nome ??
                          "Sem status",
                      )}
                    </dd>
                  </div>
                  <div className="flex items-center justify-between gap-4 border-b border-[var(--color-neutral-100)] pb-2">
                    ...
                    <dd className="font-semibold text-[var(--color-primary-900)]">
                      {formatCurrencyBRL(
                        overviewQuery.data.processo.valorEstimado,
                      )}
                    </dd>
                  </div>
                </dl>
              ...
              <SectionCard
                ...
                description="Acompanhamento resumido das principais etapas para gestores."
              >
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                          <div className="font-semibold text-[var(--color-primary-900)]">
                            {cleanDisplayText(etapa.label)}
                          </div>
                          <div className="text-sm text-[var(--color-neutral-600)]">
                            {cleanDisplayText(etapa.detalhe)}
                          </div>
                        </div>
                        <span
                          className={[
              ...
              <SectionCard
                ...
                description="Histórico recente para entender onde o processo avançou ou parou."
              >
                <div className="space-y-3">
                  {overviewQuery.data.timeline.map((row, index) => (
                    <article
                      key={`${row.moduloDestino}-${index}`}
                      className="rounded-[28px] border border-[rgba(204,225,255,0.92)] bg-[var(--color-primary-50)] px-4 py-4"
                    >
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                          ...
                          <div className="text-sm text-[var(--color-neutral-600)]">
                            {cleanDisplayText(
                              row.observacao ?? "Sem observação adicional.",
                            )}
                          </div>
                        </div>
                  ...
                  {!overviewQuery.data.timeline.length ? (
                    <Alert variant="info">
                      Ainda não há movimentações registradas para este processo.
                    </Alert>
                  ) : null}
                </div>
              </SectionCard>

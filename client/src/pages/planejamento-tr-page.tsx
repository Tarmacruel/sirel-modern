...
import {
  deletePlanejamentoDocumento,
  resolveServerAssetUrl,
  uploadPlanejamentoDocumento,
...
function extractFileName(documento: {
...
export function PlanejamentoTrPage({ processoId }: PlanejamentoTrPageProps) {
  ...
  const [supportUpload, setSupportUpload] =
  ...
  const saveMutation = trpc.planejamento.saveTr.useMutation({
    onSuccess: async () => {
      ...
      setMessage(
        form.concluir
          ? "TR externo concluído com sucesso."
          : "Etapa do TR salva em elaboração.",
      );
    },
    onError: (error) => {
      setMessage(null);
  ...
  const generateMutation = trpc.planejamento.generateDocumento.useMutation({
    onSuccess: async (created) => {
      ...
      setDocumentMessage(
        `Documento persistido no processo: ${created.titulo}.`,
      );
    },
    onError: (error) => {
  ...
  async function handleUpload(
    ...
    const setPending =
      type === "SUPORTE_TR" ? setUploadingSupport : setUploadingTr;
    ...
    try {
      ...
      setMessage(
        type === "SUPORTE_TR"
          ? "Documento de suporte do TR anexado."
          : "Documento principal do TR anexado.",
      );
    } catch (error) {
      setMessage(null);
      setErrorMessage(
  ...
  async function persistTr(concluir: boolean, silent = false) {
    ...
    if (!parsed.success) {
      ...
      if (!silent) {
        setMessage(null);
        setErrorMessage(
          "Revise os campos destacados antes de salvar a etapa do TR.",
        );
      }
      return false;
    }
  ...
  if (detailQuery.error || documentosQuery.error || !detalhe) {
    return (
      <Alert variant="warning">
        Falha ao carregar a etapa do TR no Planejamento.
      </Alert>
    );
  }

  const itens = detalhe.itens ?? [];
  const itensComReferencia = detalhe.mapaComparativo.filter(
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-3 rounded-[28px] border border-slate-200 bg-white px-5 py-5 shadow-sm">
        <Breadcrumb
          items={[
            ...
            { label: `TR ${detalhe.processo.numeroSirel}` },
          ]}
        />
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h1 className="mt-1 text-2xl font-black tracking-tight text-slate-950">
              TR externo do processo {detalhe.processo.numeroSirel}
            </h1>
            <p className="mt-2 text-sm leading-6 text-slate-600">
              Nesta etapa o TR oficial será anexado como documento externo. O
              sistema continua gerando um documento-base em HTML/PDF a partir da
              DFD, do ETP e do mapa comparativo.
            </p>
          </div>
      ...
      >
        ...
        <div className="space-y-6">
          {!detalhe.dfd?.concluido ? (
            <Alert variant="warning">
              Conclua a DFD antes de avançar para a etapa do TR.
            </Alert>
          ) : null}
          {!detalhe.etp?.concluido ? (
            <Alert variant="warning">
              Conclua o ETP antes de avançar para a etapa do TR.
            </Alert>
          ) : null}
          {!itensComReferencia.length ? (
            <Alert variant="warning">
              Registre cotações preliminares válidas antes de concluir o TR.
            </Alert>
          ) : null}
          {message ? <Alert variant="success">{message}</Alert> : null}
          {errorMessage ? <Alert variant="error">{errorMessage}</Alert> : null}
          {documentMessage ? (
            <Alert variant="success">{documentMessage}</Alert>
          ) : null}
          {documentError ? (
            <Alert variant="error">{documentError}</Alert>
          ) : null}
              ...
              <div className="space-y-4">
                <div className="grid gap-4 lg:grid-cols-4">
                  ...
                  <article className="rounded-3xl border border-slate-200 bg-slate-50 px-4 py-4">
                    ...
                    <p className="mt-2 text-xl font-black text-slate-950">
                      {formatIntegerBR(itens.length)}
                    </p>
                    <p className="mt-1 text-sm text-slate-600">
                      Itens reaproveitados da DFD para o TR.
                    </p>
                  </article>
                  <article className="rounded-3xl border border-slate-200 bg-slate-50 px-4 py-4">
                    <p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">
                    ...
                    <p className="mt-2 text-xl font-black text-slate-950">
                      {formatIntegerBR(supportDocs.length)}
                    </p>
                    <p className="mt-1 text-sm text-slate-600">
                      Documentos auxiliares anteriores ao TR principal.
                    </p>
                  </article>
                  <article className="rounded-3xl border border-slate-200 bg-slate-50 px-4 py-4">
                    <p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">
                    ...
                    <p className="mt-1 text-sm text-slate-600">
                      Valor consolidado a partir do mapa comparativo.
                    </p>
                  </article>
                </div>
                ...
                <div className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
                      ...
                      <label className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700">
                        <Checkbox
                          ...
                          onChange={(event) =>
                            setForm((current) => ({
                              ...current,
                              orcamentoSigiloso: event.target.checked,
                            }))
                          }
                        />
                  ...
                  <div className="rounded-3xl border border-slate-200 bg-slate-50 px-4 py-4">
                    ...
                    <p className="mt-2 text-lg font-black text-slate-950">
                      {detalhe.tr?.concluido ? "Concluída" : "Em elaboração"}
                    </p>
                    <p className="mt-2 text-sm text-slate-600">
                      Para concluir o TR é obrigatório anexar ao menos um
                      documento principal do TR e manter o mapa comparativo
                      válido.
                    </p>
                    <p className="mt-2 text-sm text-slate-600">
                      Orçamento sigiloso na geração:{" "}
                      <span className="font-semibold text-slate-900">
                        {form.orcamentoSigiloso ? "Sim" : "Não"}
                      </span>
                    </p>
                    <div className="mt-4 flex flex-wrap gap-3">
                      <Button
                        type="button"
                        onClick={() => void persistTr(false)}
                        disabled={saveMutation.isPending || !detalhe.dfd}
                      >
                        {saveMutation.isPending && !form.concluir
                          ? "Salvando..."
                          : "Salvar etapa"}
                      </Button>
                      <Button
          ...
          <section ref={supportRef}>
            ...
            >
              <div className="space-y-5">
                <form
                  className="grid gap-4 rounded-3xl border border-slate-200 bg-slate-50 px-4 py-4 xl:grid-cols-[1fr_1fr_220px] xl:items-end"
                  onSubmit={(event) => void handleUpload(event, "SUPORTE_TR")}
                >
                  <FormField label="Título do documento">
                    <Input
                      value={supportUpload.titulo}
                      onChange={(event) =>
                        setSupportUpload((current) => ({
                          ...current,
                  ...
                  <FormField label="Arquivo">
                    <Input
                      type="file"
                      onChange={(event) =>
                        handleFileChange(event, setSupportUpload)
                      }
                    />
                  </FormField>
                  <div className="xl:col-span-3 flex flex-wrap gap-3">
                    <Button
                      type="submit"
                      disabled={uploadingSupport || !detalhe.etp?.concluido}
                    >
                      {uploadingSupport
                        ? "Enviando suporte..."
                        : "Anexar suporte"}
                      ...
                      {supportDocs.map((documento) => (
                        <TableRow key={documento.id}>
                          <TableCell className="align-top font-semibold text-slate-900">
                            {extractFileName(documento)}
                          </TableCell>
                          <TableCell className="align-top text-slate-600">
                            {documento.descricao || "-"}
                          </TableCell>
                          <TableCell className="align-top text-slate-600">
                            {formatShortDateTimeBR(documento.criadoEm)}
                          </TableCell>
                          <TableCell className="align-top">
                            <div className="flex justify-end gap-2">
                              ...
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  void handleDeleteDocumento(documento.id)
                                }
                              >
          ...
          <section ref={trRef}>
            ...
            >
              <div className="space-y-5">
                <form
                  className="grid gap-4 rounded-3xl border border-slate-200 bg-slate-50 px-4 py-4 xl:grid-cols-[1fr_1fr_220px] xl:items-end"
                  onSubmit={(event) => void handleUpload(event, "TR_EXTERNO")}
                >
                  ...
                  <FormField label="Arquivo do TR">
                    <Input
                      type="file"
                      onChange={(event) => handleFileChange(event, setTrUpload)}
                    />
                  </FormField>
                  <div className="xl:col-span-3 flex flex-wrap gap-3">
                    <Button
                      type="submit"
                      disabled={uploadingTr || !detalhe.etp?.concluido}
                    >
          ...
          <section ref={resumoRef}>
              ...
              action={
                <div className="flex flex-wrap gap-3">
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => void persistDocumento("HTML")}
                  ...
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => void persistDocumento("PDF")}
                    disabled={generateMutation.isPending}
                  >
            ...
            >
              <div className="space-y-4">
                <Alert variant="info">
                  Este documento-base não substitui o TR externo anexado. Ele
                  serve como consolidação estruturada da fase de Planejamento e
                  pode ser arquivado no processo em HTML ou PDF.
                </Alert>
                    ...
                    <TableBody>
                      {itens.map((item) => (
                        <TableRow key={item.id}>
                          <TableCell className="align-top font-semibold text-slate-950">
                            {item.numeroItem}
                          </TableCell>
                          <TableCell className="align-top">
                            {item.descricao}
